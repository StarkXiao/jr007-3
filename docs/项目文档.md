...
队列默认不再只靠审核员自行领取，系统按审核员画像做**加权随机派单**（人工领取仍然保留）：

- **权重 = 分类偏好 × 通过率因子 × 负载因子 × 手动修正 × 加派倍数**
  - *分类偏好*：审核员显式声明的偏好分类命中 ×2；否则取近 90 天该分类的处理占比，最高 ×1.5；
  - *通过率因子*：近 90 天决策经贝叶斯平滑后映射到 0.8 ~ 1.2，**只做温和倾斜**，避免"通得越多派得越多"的马太效应；样本不足向均值收敛；
  - *负载因子*：随当前在手工单数线性衰减，达到自设上限后为 0；
  - 自己提交的条目在候选阶段直接排除；
  - 加权随机而非"永远选最高"，防止同分类任务全压给一个人。
- 审核员可设置**偏好分类、在手工单上限、暂停自动派单**，并查看自己近 90 天通过率。
- 派单与领取一样走"条件 UPDATE + 30 分钟锁"，并发下不会把同一任务派给两个人。
- 定时任务 `dispatch_sweep` 每 10 分钟执行一轮：把无主 `pending` 与锁过期的 `in_review` 派出，并回收已到期的加派。

**临时加派（接管积压）**：

- 管理员选择若干用户、时长（≤ 30 天）、加派倍数（1 ~ 5）与可选的限定分类发起加派；
- 被选中的普通用户**临时获得审核员角色**并立即收到通知，系统当场按新权重重派一轮积压；
- 到期自动结束：临时角色收回；管理员也可提前结束，并选择把未完成任务**转交他人**重新派单；
- 全过程写审计日志（`dispatch.surge.start` / `dispatch.surge.end`）。
...
- **定时任务**（BullMQ 队列 + 调度）：
  ...
  - `dispatch_sweep`：每 10 分钟加权派单，并回收到期的临时加派；
...
| POST | `/moderation/claim-next` | moderator | 智能领单：按通过率/分类偏好/负载加权派一条任务 |
| GET | `/moderation/dispatch/me` | moderator | 我的派单画像（通过率、分类偏好、上限、暂停状态） |
| PATCH | `/moderation/dispatch/me` | moderator | 设置偏好分类 / 在手上限 / 暂停自动派单 |
| GET | `/moderation/dispatch/overview` | admin | 派单总览：积压、各审核员画像与在手工单、进行中的加派 |
| POST | `/moderation/dispatch/surges` | admin | 临时加派人手（自动接管积压，见 5.5.4） |
| POST | `/moderation/dispatch/surges/:id/end` | admin | 结束加派（`reassign=true` 转交未完成任务） |
