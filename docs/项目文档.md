...
待审队列除手工领取外，支持**加权派单**，按审核员画像自动匹配任务：

```
派单分 = 通过率匹配分 × 分类偏好分 × 负载均衡分 × 探索加成 × 临时人手折扣 × 容量系数
```

- **通过率匹配**：把作者信用分换算为期望通过率（信用 100 → 95%，信用 0 → 30%），审核员历史通过率越接近期望值分越高——高信用作者交给尺度宽松且稳定的审核员，低信用作者交给更敢于驳回的审核员；
- **分类偏好**：审核员在某分类审得越多分越高（自动统计，封顶 1.8）；管理员可对单个分类设置 0–2 的手工权重，权重为 0 即不再派该分类；
- **负载均衡**：每条在办任务使分数乘性衰减 0.12（保底 0.25），防止任务堆给同一个人；
- **冷启动**：已决样本不足 5 条的新审核员获得 1.25 倍探索加成，避免"没历史 → 没单 → 永远没历史"；
- 人-任务匹配分在前 8 名候选内做**加权随机**，不是永远只选最高分，兼顾探索与公平；任务侧超时 ×1.5、每级优先级 ×1.1；
- 画像统计为 90 天滚动窗口：每次决策增量更新，每天 03:50 兜底重算；画像读带 60 秒缓存，决策后主动失效。

**临时加派（积压接管）**：

- 管理员可把普通用户临时提权为审核员（1–168 小时），原角色记入 `temp_assignments.original_role`，到期或撤销后自动恢复；
- 鉴权每次请求回库读角色，提权即时生效、回收即时失效；到期回收由 SLA 巡检（每 15 分钟）与鉴权路径双重兜底，worker 不在线也不会让临时权限变成永久权限；
- 加派期间若管理员另外正式调整过该用户角色，回收时不覆盖后续任命；
- 管理员可一键把超时积压批量预分配给全部可用审核员（含临时人手），按画像打分取最高分，每人有本轮硬上限防止垄断；
- 加派、撤销、批量接管、画像调整全部写审计日志（`review.temp_assign` / `review.temp_revoke` / `review.temp_takeover` / `review.dispatch.profile`），自动派单记录 `review.dispatch` 及各因子得分。
...
| POST | `/moderation/dispatch/next` | moderator | 加权派单：按通过率/分类偏好/负载领下一条并加锁 |
| GET | `/moderation/dispatch/moderators` | moderator | 当前可派单审核员画像（通过率、分类偏好） |
| GET | `/admin/dispatch/profiles` | admin | 全部审核员画像列表 |
| PATCH | `/admin/dispatch/profiles/:uuid` | admin | 调整分类手工权重/容量系数/暂停派单 |
| GET | `/admin/dispatch/temp-assignments` | admin | 加派批次列表（可按状态过滤） |
| POST | `/admin/dispatch/temp-assignments` | admin | 临时加派（用户、时长、原因、接管上限） |
| POST | `/admin/dispatch/temp-assignments/:batchId/revoke` | admin | 撤销加派并恢复原角色 |
| POST | `/admin/dispatch/temp-assignments/expire-due` | admin | 手动收回到期加派 |
| POST | `/admin/dispatch/takeover` | admin | 批量接管积压（可指定审核员，默认全部含临时人手） |
